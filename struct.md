...
def heapsort(tree):
    ...
    tree_size = len(tree)
    # print(tree_size)
    # print(data_to_sort[1], data_to_sort[14])
    
    # 여기에 코드를 작성하세요
    root_node = 1
    
    for i in range((tree_size - 1) // 2, 0, -1):
    ...
    for i in range(tree_size - 1, 0, -1) :
        print(i)
        swap(tree, root_node, i)
        heapify(tree, root_node, i)
...
def reverse_heapify(tree, index):
    ...
    # 여기에 코드를 작성하세요
    # while parent_index > 0 and tree[parent_index] < tree[index] :
